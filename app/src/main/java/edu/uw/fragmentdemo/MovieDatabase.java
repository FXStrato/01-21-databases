...
public final class MovieDatabase {
    ...
    public static void testDatabase(Context context) {
        ...
        content.put(FavoriteEntry.COL_TITLE, "Buttman");
        content.put(FavoriteEntry.COL_YEAR, "1999");
        content.put(FavoriteEntry.COL_IMDB_ID, "6969");
        content.put(FavoriteEntry.COL_POSTER_URL, "yourmom.exe");

        try {
            long newRowId = db.insert(FavoriteEntry.TABLE_NAME, null, content) ;
            Log.v("Database", "" + newRowId);
        } catch (SQLiteConstraintException e) {
            Log.v("Database_Error", "Lol oops: " + e);
        }
